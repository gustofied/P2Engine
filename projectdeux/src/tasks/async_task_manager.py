...
class AsyncTaskManager:
    def __init__(self):
        """Initialize with the global task registry and logger."""
    ...
    def dispatch_task(self, task_name: str, args: list, kwargs: dict, queue: str = "default") -> AsyncResult:
        ...
        self.logger.log_interaction("AsyncTaskManager", "System", f"Dispatched task: {task_name} to queue: {queue}, Task ID: {async_result.id}")
        return async_result
    ...
    def dispatch_workflow(self, task_list: List[Tuple[Callable, List, str]]) -> AsyncResult:
        ...
        for i, (task_func, args, queue) in enumerate(task_list):
            if i == 0:
                # First task gets all its arguments directly
                task = task_func.s(*args).set(queue=queue)
            else:
                # Subsequent tasks get previous output plus additional args
                task = task_func.s(*args).set(queue=queue)
    ...
    def get_task_result(self, async_result: AsyncResult, timeout: int = 60):
        ...
        try:
            ...
            self.logger.log_interaction("AsyncTaskManager", "System", f"Got result: {result}")
            return result
        except Exception as e:
            self.logger.log_interaction("AsyncTaskManager", "System", f"Error getting result: {e}")
            raise