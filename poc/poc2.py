...
alias_template_id = f'{package_id}:User:Alias'  # Assuming the Alias template is in the User module

# Parties
alice_party_id = 'party-2d50054a-e7ee-46a0-a1f6-0fdf561071fa::12205d5c2964224e4254b0121a069d6b3146b430561661910afc3f9759d611045ee8'
bob_party_id = 'party-43a02a67-25e1-40cd-ba95-925927449d92::12205d5c2964224e4254b0121a069d6b3146b430561661910afc3f9759d611045ee8'
...
# Set up headers without specifying a party (for non-party-specific endpoints)
...
def fetch_contract_by_id(party_id, contract_id):
    headers = get_headers(party_id)
    fetch_payload = {
        'templateId': user_template_id,
    ...
    print(f"Fetch Contract by ID Response Status Code: {response.status_code}")
    print(f"Fetch Contract by ID Response Content: {response.text}")

# Fetch a contract by key
def fetch_contract_by_key(party_id, contract_key):
    headers = get_headers(party_id)
    fetch_payload = {
        'templateId': user_template_id,
        'key': contract_key,
    ...
    print(f"Fetch Contract by Key Response Status Code: {response.status_code}")
    print(f"Fetch Contract by Key Response Content: {response.text}")

# Create and exercise a choice immediately
def create_and_exercise(party_id):
    headers = get_headers(party_id)
    create_exercise_payload = {
        'templateId': user_template_id,
        'payload': {
            'username': party_id,
            'following': [],
    ...
    response = requests.post(
        f'{base_url}/v1/create-and-exercise',
        headers=headers,
        json=create_exercise_payload,
    )
    print(f"Create and Exercise Response Status Code: {response.status_code}")
    print(f"Create and Exercise Response Content: {response.text}")

    if response.status_code == 200:
        print(f"{party_id} successfully created a User contract and followed Bob.")
    else:
        print(f"Failed to create and exercise for {party_id}.")
...
# Download a DAR package
def download_dar_package(package_id):
    headers = get_headers_no_party()
    response = requests.get(
        f'{base_url}/v1/packages/{package_id}',
        headers=headers,
    )
    print(f"Download DAR Package Response Status Code: {response.status_code}")
    # Save the content to a file if needed
    if response.status_code == 200:
        with open(f'{package_id}.dar', 'wb') as f:
            f.write(response.content)
        print(f"DAR package {package_id} saved to {package_id}.dar")
    else:
        print(f"Failed to download DAR package {package_id}")

# Main function
def main():
    # Replace 'your_namespace' with your actual namespace
    alice_party_id_actual = 'party-2d50054a-e7ee-46a0-a1f6-0fdf561071fa::your_namespace'
    bob_party_id_actual = 'party-43a02a67-25e1-40cd-ba95-925927449d92::your_namespace'
    ...
    # Allocate a new party
    print("\nAllocating a new party 'Charlie'...")
    allocate_new_party(alice_party_id, 'Charlie')
    charlie_party_id = 'Charlie::your_namespace'  # Update with your actual namespace

    # List known parties
    print("\nListing known parties...")
    list_known_parties(alice_party_id)
    ...
    print("\nGetting or creating User contracts for Alice and Bob...")
    ...
    # Fetch Alice's contract by ID
    print("\nFetching Alice's User contract by ID...")
    fetch_contract_by_id(alice_party_id, alice_contract_id)

    # Fetch Bob's contract by key
    print("\nFetching Bob's User contract by key...")
    fetch_contract_by_key(alice_party_id, bob_party_id)  # Assuming the key is the username

    # Create and immediately exercise a choice with Charlie
    print("\nCreating and immediately exercising a choice with Charlie...")
    create_and_exercise(charlie_party_id)

    # Query Alice's User contract
    print("\nQuerying Alice's User contracts...")
    query_user_contract(alice_party_id)
    ...
    # Download DAR package
    print("\nDownloading DAR package...")
    download_dar_package(package_id)
