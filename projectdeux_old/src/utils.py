...
def start_celery_workers(queues, unique_run_id):
    ...
    Args:
        queues (set): Set of queue names to listen to (e.g., {"agent_queue_test_abc123"}).
    ...
    celery_cmd = f"{python_exe} -m celery -A celery_app worker -Q {queue_list} --loglevel=info -n worker_{unique_run_id}@%h"
