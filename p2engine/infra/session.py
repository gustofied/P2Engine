...
# Thread-local container for cached Session objects ---------------------------
...
class Session:
    ...
    def stack_for(self, agent_id: str) -> InteractionStack:
        ...
        # Inject a synthetic seed message when a branch is first created and
        # not yet terminated.  Prevents empty-branch edge-cases.
        if stack.length() == 0:
...
def get_session(conv_id: str, redis_client: redis.Redis) -> "Session":
    ...
    # Ensure the attribute exists (one dict per thread)
    ...
    if conv_id in _thread_local.sessions:
        ...
        if sess.redis is not redis_client:
            # Same conversation already opened earlier in this thread with a
            # different Redis connection object.  That is harmless – prefer
            # the cached Session and ignore the new client.
            logger.debug(
                {
